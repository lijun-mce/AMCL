...
// Forward declarations 声明
class AMCLSensorData;

// Base class for all AMCL sensors 所有AMCL传感器的基类 可以理解为只有声明，没有实现
class AMCLSensor {
  // Default constructor
public:
  AMCLSensor();
...
// Base class for all AMCL sensor measurements 所有AMCL传感器测量的基类
// AMCLSensorData 中包含 AMCLSensor， AMCLSensorDate 没有函数实现
class AMCLSensorData {
