...
// Allows AMCL to run from bag file 通过录包的形式运行
...
// Pose hypothesis 可能位姿
typedef struct
{
  // Total weight (weights sum to 1)
  double weight;

  // Mean of pose esimate 姿态估计的均值  double v[3];
  pf_vector_t pf_pose_mean;  

  // Covariance of pose estimate 姿态估计的协方差  double m[3][3]
  pf_matrix_t pf_pose_cov;

} amcl_hyp_t;

// 正则化，将所有角度都变换到-pi -- pi 之间
...
class AmclNode
{
  ...
  private:
    ...
    ros::Subscriber initial_pose_sub_;  // 接收初始位置
    ...
    ros::Publisher pose_pub_;   // 发布机器人位置
    ros::Publisher particlecloud_pub_;  // 姿态估计集合
    ros::ServiceServer global_loc_srv_;  // 全局定位服务
    ros::ServiceServer nomotion_update_srv_; //to let amcl update samples without requiring motion
    ros::ServiceServer set_map_srv_;
    ros::Subscriber initial_pose_sub_old_;  // 接收初始位置
    ros::Subscriber map_sub_;               // 接收地图信息

    // diagnostic_updater为硬件诊断模块，收集各种状态信息和硬件信息，以进行分析，故障排除和记录
        ...
        first_reconfigure_call_(true)
{
  ...
  // 在变量〜initial_pose_ 和〜initial_cov_ 中存储参数服务器的最后估计姿态和协方差的最大速率
  ...
  // 里程计模型参数，如果是差速模型，只使用噪声参数odom_alpha_1到odom_alpha4
  private_nh_.param("odom_model_type", tmp_model_type, std::string("diff"));
  if(tmp_model_type == "diff")
    odom_model_type_ = ODOM_MODEL_DIFF;
  else if(tmp_model_type == "omni")  // 全向轮模型 odom_alpha_1到odom_alpha5
...
// 接收激光数据 里面包含了通过里程计位移更新每一个粒子的位姿，通过激光雷达数据映射到地图中
// 给每个粒子评分
void AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  // If the robot has moved, update the filter 机器人移动后更新每个粒子的位姿
  else if(pf_init_ && lasers_update_[laser_index])
  {
    ...
    // Use the action data to update the filter 调用里程计模型更新每一个粒子的位姿
  ...
  if(lasers_update_[laser_index])
  {
    ...
    // 考虑到激光是倒置安装，我们确定最小，最大，增量角度的激光在基本框架。
    // Construct min and max angles of laser, in the base_link frame. 在base_link帧中构造最小和最大的激光角度。
    tf2::Quaternion q;
    ...
    // Resample the particles 重新取样的粒子
  ...
  ///////////////////////////////////////////////////////
   ...
   // resampled= force_publication= false
 ...
 * 如果initial_pose_hyp_和map_都是非空的，应用初始姿态到粒子过滤器状态。
 * initial_pose_hyp_被使用后被删除，并设置为NULL。
 */
